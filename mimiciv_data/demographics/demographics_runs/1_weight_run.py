...
def run_query(query_file, output_csv=None):
    ...
    weight_path = os.path.join(derive_path, 'weight_durations.csv')
    weight = pd.read_csv(weight_path)

    target_patients.to_sql('target_patients', conn, index=False, if_exists='replace')
    weight.to_sql('weight_durations', conn, index=False, if_exists='replace')
...
if __name__ == "__main__":
    query_file = DEMO_QUERIES_DIR / 'weight_query.sql'
    
    output_csv = DEMO_RESULTS_DIR / 'weight_output.csv'
