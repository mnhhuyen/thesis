WITH cbc AS (
    ...
    LEFT JOIN blood_count bc
        ON bc.subject_id = tp.subject_id
            AND bc.charttime >= DATETIME(tp.intime, '-6 hour')
            AND bc.charttime <= DATETIME(tp.intime, '+1 day')
    GROUP BY tp.stay_id
)
...
, chem AS (
    SELECT
        ...
        , MIN(magnesium) AS magnesium_min, MAX(magnesium) AS magnesium_max
        , MIN(chloride) AS chloride_min, MAX(chloride) AS chloride_max
    FROM target_patients tp
    LEFT JOIN chemistry ch
        ON tp.subject_id = ch.subject_id
            AND ch.charttime >= DATETIME(tp.intime, '-6 hour')
            AND ch.charttime <= DATETIME(tp.intime, '+1 day')
    GROUP BY tp.stay_id
)
...
, coag AS (
    ...
    FROM target_patients tp
    LEFT JOIN coagulation co
        ON co.subject_id = tp.subject_id
            AND co.charttime >= DATETIME(tp.intime, '-6 hour')
            AND co.charttime <= DATETIME(tp.intime, '+1 day')
    GROUP BY tp.stay_id
)
...
, enz AS (
    SELECT
        ...
        , MIN(ck_cpk) AS ck_min, MAX(ck_cpk) AS ck_max
    ...
    FROM target_patients tp
    LEFT JOIN enzyme ez
        ON tp.subject_id = ez.subject_id
            AND ez.charttime >= DATETIME(tp.intime, '-6 hour')
            AND ez.charttime <= DATETIME(tp.intime, '+1 day')
    GROUP BY tp.stay_id
)
...
, cardiac AS (
    SELECT
        ...
        , MIN(troponin_t) AS troponin_t_min, MAX(troponin_t) AS troponin_t_max

    FROM target_patients tp
    LEFT JOIN cardiac_marker cm
        ON tp.subject_id = cm.subject_id
            AND cm.charttime >= DATETIME(tp.intime, '-6 hour')
            AND cm.charttime <= DATETIME(tp.intime, '+1 day')
    GROUP BY tp.stay_id
)
...
SELECT
    ...
    -- cardiac marker
    , ntprobnp_min, ntprobnp_max
    , troponin_t_min, troponin_t_max
FROM target_patients tp
LEFT JOIN cbc
    ON tp.stay_id = cbc.stay_id
LEFT JOIN chem
    ON tp.stay_id = chem.stay_id
-- LEFT JOIN diff
--     ON ie.stay_id = diff.stay_id
LEFT JOIN coag
    ON tp.stay_id = coag.stay_id
LEFT JOIN enz
    ON tp.stay_id = enz.stay_id
LEFT JOIN cardiac
    ON tp.stay_id = cardiac.stay_id
;