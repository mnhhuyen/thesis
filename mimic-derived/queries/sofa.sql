...
WITH co AS (
  ...
  SELECT
    co.stay_id,
    co.hr,
    MIN(vts.mbp) AS meanbp_min
  FROM co
  LEFT JOIN derived.vitalsigns AS vts
    ON co.stay_id = vts.stay_id
    AND co.starttime < vts.charttime
    AND co.endtime >= vts.charttime
  GROUP BY
    co.stay_id,
    co.hr
), gcs AS (
  ...
  SELECT
    co.stay_id,
    co.hr,
    MIN(bc.platelet) AS platelet_min
  FROM co
  LEFT JOIN derived.blood_count AS bc
    ON co.hadm_id = bc.hadm_id
    AND co.starttime < bc.charttime
    AND co.endtime >= bc.charttime
  GROUP BY
    co.stay_id,
    co.hr
), pf AS (
