DROP TABLE IF EXISTS vitalsigns; CREATE TABLE vitalsigns AS
SELECT
  ce.subject_id,
  ce.stay_id,
  ce.charttime,
  ...
  AVG(CASE WHEN itemid IN (228151) THEN valuenum END) AS map_dias,

  AVG(CASE WHEN itemid IN (228152) THEN valuenum END) AS map_sys,
  AVG(
    CASE
...
WHERE
  ...
  AND ce.itemid IN (220045, 225309, 225310, 225312, 220050, 220051, 220052, 220179, 220180, 220181, 220210, 224690, 220277, 225664, 220621, 226537, 223762, 223761, 224642, 228151, 228152)
GROUP BY
  ce.subject_id,
  ce.stay_id,
  ce.charttime