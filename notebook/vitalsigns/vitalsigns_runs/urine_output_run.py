...
def run_query(query_file, output_csv=None):
    ...
    urine_outputs = pd.read_csv(os.path.join(derive_path, 'urine_output.csv'))

    target_patients.to_sql('target_patients', conn, index=False, if_exists='replace')
    urine_outputs.to_sql('urine_output', conn, index=False, if_exists='replace')
...
if __name__ == "__main__":
    query_file = VITAL_QUERIES_DIR / 'urine_output_query.sql'
    output_csv = VITAL_RESULTS_DIR / 'urine_output_output.csv'
